...
//  Swift语法
//
//  Created by caowei on 2020/10/13.
...
/**
 ...
 使代码更加模块化：不透明类型可以帮助开发者抽象出通用行为，并隐藏具体实现细节，从而使代码更加模块化，易于维护和修改。

 增强程序的安全性：不透明类型可以防止程序的使用者直接获取类型具体的实现信息，从而增强程序的安全性。
 ...
 其中，some关键字用于声明一个不透明类型，并表示返回的值类型实现了某个指定的协议。在具体实现过程中，函数可以返回任何实现了该协议的具体类型，但是函数的使用者无法获取该类型的具体信息，只能通过协议来使用该类型的公共接口。

 不透明类型可以用于多个方面，例如函数返回值、变量、参数、属性等。使用不透明类型可以提高代码的模块化程度，使得代码更易于维护和修改。

 */

import UIKit

class UnClearType: UIViewController {
...
struct FlippedShape<T: Shape22>: Shape22 {
    var shape: T
    func draw() -> String {
...
struct JoinedShape<T: Shape22, U: Shape22>: Shape22 {
    var top: T
    var bottom: U
    func draw() -> String {
        return top.draw() + "\n" + bottom.draw()
...
// 在协议前面标记上 some 后，返回值的类型对编译器就变成透明的了。在这个值使用的时候编译器可以根据返回值进行类型推断得到具体类型。 https://juejin.im/post/6844903862290104327⚠️
func makeTrapezoid() -> some Shape22 {
    let top = Triangle(size: 2)
    let middle = Square22(size: 2)
...
func flip<T: Shape22>(_ shape: T) -> some Shape22 {
    return FlippedShape(shape: shape)
}
func join<T: Shape22, U: Shape22>(_ top: T, _ bottom: U) -> some Shape22 {
...
//
//func invalidFlip<T: Shape22>(_ shape: T) -> some Shape22 {
//    if shape is Square {
...
struct FlippedShape2<T: Shape22>: Shape22 {
    var shape: T
    func draw() -> String {
        if shape is Square {
            return shape.draw()
...
func protoFlip<T: Shape22>(_ shape: T) -> Shape22 {
    return FlippedShape(shape: shape)
}
...
func protoFlip2<T: Shape22>(_ shape: T) -> Shape22 {
    if shape is Square22 {
        return shape
    }
...
// 错误：有关联类型的协议不能作为返回类型。⚠️
//func makeProtocolContainer<T>(item: T) -> Container3 {
//    return [item]
//}

// 错误：没有足够多的信息来推断 C 的类型。
//func makeProtocolContainer<T, C: Container3>(item: T) -> C {
//    return [item]
//}


//而使用不透明类型 some Container 作为返回类型，就能够明确地表达所需要的 API 契约 —— 函数会返回一个集合类型，但并不指明它的具体类型：⚠️
func makeOpaqueContainer<T>(item: T) -> some Container3 {
    return [item]
}
